...
h = legend([plot_lowQME, plot_lowF0, ...
             ...
             TOD], ...
             ['    QM: $c_{opt}$ = ' num2str(x_min_low1) '$\AA$;' '    SCF cutoff $10^{-6}$ - Plane wave cutoff 600'], ...
             ['QM+phn: $c_{opt}$ = ' num2str(x_min_low2) '$\AA$;' '    SCF cutoff $10^{-6}$ - Plane wave cutoff 600'], ...
             ['    QM: $c_{opt}$ = ' num2str(x_min_med1) '$\AA$;' '    SCF cutoff $10^{-8}$ - Plane wave cutoff 600'], ...
             ['QM+phn: $c_{opt}$ = ' num2str(x_min_med2) '$\AA$;' '    SCF cutoff $10^{-8}$ - Plane wave cutoff 600' ' False phonon'], ...
             ['    QM: $c_{opt}$ = ' num2str(x_min_hig1) '$\AA$;' '    SCF cutoff $10^{-8}$ - Plane wave cutoff 800'], ...
             ['QM+phn: $c_{opt}$ = ' num2str(x_min_hig2) '$\AA$;' '    SCF cutoff $10^{-8}$ - Plane wave cutoff 800'], ...
             ['Tod: $c_{opt}$ = 6.6721 $\AA$'] ...
 );
...
h = xlabel('Lattice constant c/ $\AA$');
set(h,'interpreter','latex');
h = ylabel('Phonon Correction/ $eV$ per atom');
set(h,'interpreter','latex');
