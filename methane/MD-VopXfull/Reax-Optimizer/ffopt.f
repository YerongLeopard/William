...
**********************************************************************   
      ...
      if (iopt.eq.0) then
       ...
       if (imodfile.eq.0) call writebgf(88)
       if (imodfile.eq.1.and.ngeofor.eq.1) call writebgf(88)
       close (88)
       if (imodfile.eq.0) qfile2=qfilh(istart:iend-1)//".geo"
       open (88,file=qfile2,status='unknown')
       if (imodfile.eq.0) call writegeo(88)
       if (imodfile.eq.1.and.ngeofor.eq.0) call writegeo(88)
       close (88)
       end if
       end do
       end if
...
*     read (21,*)ichn(1),ichn(2),ichn(3),vchange,vpmax,vpmin
      ...
      if (vchange.lt.0.00001) vchange=0.00001
      if (vpmax.lt.0.001.and.vpmax.gt.-0.001) vpmax=25000.0
      if (vpmin.lt.0.001.and.vpmin.gt.-0.001) vpmin=-25000.0
      icount=0
...
*     call bepsdy(3)
      ...
      if (sdy(1).lt.errsav) call ffchng(1,4,vpmax,vpmin)
      if (sdy(2).lt.errsav) call ffchng(2,4,vpmax,vpmin)
     ...
     $and.ngeofor.eq.0) call cpfile(98,3)
      if (ingeo.eq.1.and.sdy(3).lt.errsav.and.nsurp.ne.2.
     $and.ngeofor.eq.1) call cpfile(90,3)
      if (nsurp.ge.2) then
      rewind (90)
      rewind (98)
      do i1=1,nmollset
      ...
      if (ngeofor.eq.0) call writegeo(88)
      if (ngeofor.eq.1) call writebgf(88)
      close(88)
      end if
      end do
...
**********************************************************************   
      ...
      read (nunit,1260)(fpar(2,i1,i2),i2=33,35)
      end do
      ihulp=2+npar+4
      ...
      do i1=1,nodmty
      read (nunit,1400)idmo(i1,1),idmo(i1,2),(fpar(4,i1,i2),i2=1,11)
      end do
...
**********************************************************************   
   30 continue
      ...
      write (nunit,1260)(fpar(2,i1,i2),i2=33,35)
*      write (nunit,1250)(fpar(2,i1,i2),i2=33,40)
      end do
      ihulp=2+npar+4
      ...
      do i1=1,nodmty
      write (nunit,1400)idmo(i1,1),idmo(i1,2),(fpar(4,i1,i2),i2=1,11)
      end do
...
**********************************************************************
      ...
      if (ifound.eq.0) then
      write (99,'(a60,5f12.4)')qrom2(1:80),caldat(i2),
     $compdat(i2),weightdat(i2),error*error,toterr
      else
      write (99,'(a60,5f12.4)')qrom2(22:80),caldat(i2),
     $compdat(i2),weightdat(i2),error*error,toterr
      end if
...
*    $compdat(i2),weightdat(i2),error*error,toterr
      ...
      error=(caldat(i2)-compdat(i2))/weightdat(i2)
      toterr=toterr+error*error
      write (99,'(a60,5f12.4)')qdatid(i2),caldat(i2),
     $compdat(i2),weightdat(i2),error*error,toterr
      end if
...
**********************************************************************   
      ...
      !DEBUG
      !open (38,file='DEBUG',status='unknown',access='append')
      !write (38,*) 'DEBUG'      
      !close (38)
      qlab(1)='Bond length      '
...
**********************************************************************   
      ...
      if (abs(sdy(1)-sdy(2)).lt.1e-5.and.abs(sdy(1)-sdy(3)).lt.1e-5)
     $then
      temax=stem(3)
      goto 10
      end if
      ...
      ste22=stem(2)*stem(2)
      ste12=stem(1)*stem(1)
      if ((ste22-ste12).lt.1e-5) ste22=ste22+0.0010   !Avoid problems with parabolic search
...
*     write (nunit1,1700)qkeyw(nprob)
      ...
      if (ipropt.eq.1) write (nunit1,2037)vpresopt,vprestax,
     $vprestay,vprestaz
      if (ipropt.eq.2) write (nunit1,2038)
...
*     if (qr.eq.'F'.or.qr.eq.'5'.or.qr.eq.'Y') 
     ...
     $angles(2),angles(3)
      ...
      if (neqdis.gt.0) write (nunit1,2250)
      do i2=1,neqdis
      write (nunit1,2260)
