...
********************************************************************** 
     ...
     $lnpecan,ltmpdamp
      !open (38,file='fort.38',status='unknown',access='append')
      !write (38,*) 'In reac'
      !close (38)
      if (ndebug.eq.1) then
...
***********************************************************************
      ...
      if (inpt.eq.1.or.inpt.eq.4) scalex=1.0
...
**********************************************************************
      ...
      dimension dispc6i(nsort),disprei(nsort),scalslgi(nsort)
**********************************************************************
*                                                                    *
*     Read in force field                                            *
*                                                                    *
...
**********************************************************************
     ...
     $'Warning: very low value for upper Taper-radius cutoff:',swb
      dispscale=vpar(36)
      if (dispscale.lt.0.5) write (*,*)
     $'Warning: scale factor in dispersion too small:',dispscale
      dispscale2=vpar(23)
      if (dispscale2.lt.0.5) write (*,*)
     $'Warning: scale factor in dispersion too small:',dispscale2
**********************************************************************
*                                                                    *
...
**********************************************************************
     ...
     $bo131(i1),bo132(i1),bo133(i1),sigqeq(i1),default
      iline=iline+1
      read (4,1250,end=990,err=990)vovun(i1),vval1(i1),vlp2(i),
...
**********************************************************************
      ...
      scalslg(i1,i2)=sqrt(scalslgi(i1)*scalslgi(i2))
...
**********************************************************************
     ...
     $rsig,rpi,rpi2,dispc6ij,codmh,sodmh,dodmh,sedmh
      iline=iline+1
      if (dispc6ij.ge.zero) dispc6(nodm1,nodm2)=dispc6ij
      if (dispc6ij.ge.zero) dispc6(nodm2,nodm1)=dispc6ij
...
* ReaxFP start
*      if (rpi.gt.zero) fp1co(nodm1,nodm2)=1.0
*      if (rpi.gt.zero) fp1co(nodm2,nodm1)=1.0
* ReaxFP end
...
* ReaxFP start 
*      if (rodmh.gt.zero) p1co(nodm1,nodm2)=1.0*rodmh
*      if (rodmh.gt.zero) p1co(nodm2,nodm1)=1.0*rodmh
      p1co(nodm1,nodm2)=1.0*rodmh
      p1co(nodm2,nodm1)=1.0*rodmh
* ReaxFP end
...
*      if (godmh.gt.zero) p3co(nodm2,nodm1)=godmh
      p2co(nodm1,nodm2)=deodmh
      p2co(nodm2,nodm1)=deodmh
      p3co(nodm1,nodm2)=godmh
      p3co(nodm2,nodm1)=godmh
* ReaxFP start
      p4co(nodm1,nodm2)=1.0*codmh
      p4co(nodm2,nodm1)=1.0*codmh
      p5co(nodm1,nodm2)=1.0*sodmh
      p5co(nodm2,nodm1)=1.0*sodmh
      p6co(nodm1,nodm2)=1.0*dodmh
      p6co(nodm2,nodm1)=1.0*dodmh
      p7co(nodm1,nodm2)=1.0*sedmh
      p7co(nodm2,nodm1)=1.0*sedmh
* ReaxFP end
      end do
**********************************************************************
*                                                                    *
...
* SN ReaxPQ
      read (4,1500,end=990,err=990)nhbs(i1,1),nhbs(i1,2),
     $nhbs(i1,3),rhb(i1),dehb(i1),vhb1(i1),vhb2(i1),vhbtet0(i1)
* End SN 
      iline=iline+1
      end do
**********************************************************************
...
**********************************************************************
 ...
 1260 format (3x,3f9.4)
 1300 format (f10.4)
 1400 format (2i3,11f9.4)
...
* reaxfp
      estrc=ncha5*eb+ncha3*ea+ncha6*elp+ncha9*ev+ncha10*ecoa+
     $ncha7*emol+epen+ncha12*et+ncha11*ehb+ncha13*eco+ncha4*ew+
     $ncha1*ep+epist+ncha2*ech+efi
...
* ReaxFP start 
      ...
      rh2=zero
      if (rat(ih1).gt.zero.and.rat(ih2).gt.zero) then
      rh2=bop1(itype)/(rob1(ih1,ih2)-bop2(itype))
      rhulp=exp(-rh2*(dis-bop2(itype)))
      ehulp=(1.1+0.0001)*1/(1+rhulp)
      if (ehulp.lt.0.85)  ehulp = 0.0
     
      end if

      bor=ehulp+ehulpp+ehulppp
*      write(*,*) 'i j bo ', i1, i2, bor
      borsi=ehulp
      borpi=ehulpp
      borpi2=ehulppp

      dbordrob= rh2*rhulp/(1+rhulp)**2
*     we have to add for p and pp
      dborsidrob = rh2*rhulp/(1+rhulp)**2
      dborpidrob = zero
      dborpi2drob = zero 
...
************************************************************************
     ...
     $(bodum(i2),i2=1,25-iag(i1,2)),abo(i1),vlp(i1),ch(i1)
      ...
      write(*,*) 'test1 nsbmax',nsbmax
      write (*,*)i1,iag(i1,1),(iag(i1,2+i2),i2=1,iag(i1,2)),
     $(idum(i2),i2=1,35-iag(i1,2)),
     $iag(i1,3+mbond),(bo(nubon1(i1,i2)),i2=1,iag(i1,2)),
     $(bodum(i2),i2=1,35-iag(i1,2)),abo(i1),vlp(i1),ch(i1)
      write (7,250)i1,iag(i1,1),(iag(i1,2+i2),i2=1,iag(i1,2)),
     $(idum(i2),i2=1,143-iag(i1,2)),
     $iag(i1,3+mbond),(bo(nubon1(i1,i2)),i2=1,iag(i1,2)),
     $(bodum(i2),i2=1,143-iag(i1,2)),abo(i1),vlp(i1),ch(i1)
!      write (7,250)i1,iag(i1,1),(iag(i1,2+i2),i2=1,iag(i1,2)),
!     $(idum(i2),i2=1,35-iag(i1,2)),
!     $iag(i1,3+mbond),(bo(nubon1(i1,i2)),i2=1,iag(i1,2)),
!     $(bodum(i2),i2=1,35-iag(i1,2)),abo(i1),vlp(i1),ch(i1)
...
* ReaxFP start 
      if (qident.eq.'ichaco') ncha1=int(vhulp)
      if (qident.eq.'iatmco') ncha3=int(vhulp)
      if (qident.eq.'ivdwco') ncha4=int(vhulp)
      if (qident.eq.'iebond') ncha5=int(vhulp)
      if (qident.eq.'ilonep') ncha6=int(vhulp)
      if (qident.eq.'imolec') ncha7=int(vhulp)
      if (qident.eq.'ivdwco') ncha8=int(vhulp)
      if (qident.eq.'iangle') ncha9=int(vhulp)
      if (qident.eq.'iconan') ncha10=int(vhulp)
      if (qident.eq.'ihydbo') ncha11=int(vhulp)
      if (qident.eq.'itorsi') ncha12=int(vhulp)
      if (qident.eq.'iconju') ncha13=int(vhulp)
...
************************************************************************
   ...
   12 continue
      ...
      write (qdatid(ndata2),180)qkeyhulp
*      write (qdatid(ndata2),180)qkeyhulp,iatn
      end if
      if (ihulp.eq.2) then
      caldat(ndata2)=presy/1000.0
      write (qdatid(ndata2),181)qkeyhulp
      end if
      if (ihulp.eq.3) then
      caldat(ndata2)=presz/1000.0
      write (qdatid(ndata2),182)qkeyhulp
      end if
      if (ihulp.eq.4) then
      caldat(ndata2)=pressu/1000.0
      write (qdatid(ndata2),183)qkeyhulp
      end if
...
************************************************************************
   15 read (63,'(a80)',err=9010,end=500)qhulp
      ...
      write(*,*) "weight,iatn,vcomp ",weight,iatn,vcomp
