...
********************************************************************** 
      ...
      j1=ib(i1,2)
      j2=ib(i1,3)
*      write(*,*) '0. bond i j boi ', i1,j1,j2,bo(i1)
...
*     ovj2=aboj-valf(itj)
      ...
      ovj2=aboj-vval3(itj)
*      write(*,*) "ovi ovj ",ovi2,  ovj2,"vval3(i) vval3j
*     $",vval3(iti),vval3(itj)
*     ovi2=aboi-aval(iti)
*     ovj2=aboj-aval(itj)
...
*     end do
      ...
      ovi2=aboi-vval3(iti)
      ovj2=aboj-vval3(itj)
*      if (ovi2.le.0.00)  bocor1=1.0
*       if (ovj2.le.0.00)  bocor2=1.0
*      write(*,*) 'vval3 i j ', vval3(iti),vval3(itj)
      exphu5=1.0
      exphu6=1.0
      if (ovi2.gt.0.00) exphu5=vval3(iti)/aboi
      if (ovj2.gt.0.00) exphu6=vval3(itj)/aboj
      exphu7=min(exphu5,exphu6)
*      write(*,*) '1. i j boi aboi aboj', i1,j1,j2,bo(i1),aboi,aboj
      exphu7=1.0
*      if (j1.eq.1.and.j2.eq.5) exphu7=0.0
      bo(i1)=boo*exphu7
...
*  ReaxFP start 
      ...
      exphu1=exp(bopo1)
      exphu2=-exp(bopo2)
      ebh1 = -0.5*de1h*bom(ibt)*(exphu1+exphu2)


      ebh=ebh1-de2h*bopia-de3h*bopi2a
...
*  ReaxFP start
      debdbo=ebh1*(psi(ibt)*exphu1-psp(ibt)*exphu2)
      debdbopi=-de2h
      debdbopi2=-de3h
*  ReaxFP end
...
* SN reaxpq 
      thba=th0(ity)
      thetao = thba*dgrrdn
      cangle = 2.0*vka(ity)/sin(thetao)**2
      ibt1 = ib(la,1)
      ibt2 = ib(lb,1)
...
* ebij
      ...
      rrw=rob1(ity1,ity2)
      h1=100.0*p1*rrw/rr
      h2=exp(p3*(1.0-rr/rrw))
      de1h=de1(ibt1)
      bopo1=boa*psi(ibt1)
      bopo2=-boa*psp(ibt1)
      exphu1=exp(bopo1)
      exphu2=-exp(bopo2)
      ebij = -0.5*de1h*bom(ibt1)*(exphu1+exphu2)+p2*(1+h1)*h2 
*ejk
      a(1)=c(j(3),1)-c(j(2),1)
      a(2)=c(j(3),2)-c(j(2),2)
      a(3)=c(j(3),3)-c(j(2),3)
      ...
      exphu1=exp(bopo1)
      exphu2=-exp(bopo2)
      ebjk = -0.5*de1h*bom(ibt2)*(exphu1+exphu2)+p2*(1+h1)*h2
...
*      evh = cangle*ebij*ebjk*(cos(hl)-cos(thetao))**2
      evh = cangle*exa2*exb2*(cos(hl)-cos(thetao))**2
...
********************************************************************** 
      include 'cbka.blk'
      dimension drda(3),j(4),dvdc(3,3),dargdc(3,3)
      virial=zero
      virx=zero
      viry=zero
      virz=zero
...
* SN ReaxPQ
      ...
      do i2=1,ia(j(3),2)
      write(*,*) "i nbon j ",ia(j(3),1),ia(j(3),2),ia(j(3),2+i2)
      j(4) = ia(j(3),i2+2)
      if (nphb(ia(j(4),1)).eq.1) then
      call calvalhb(j(4),j(2),j(3),ix,iy,iz,arg,hhb(i1),dvdc,dargdc)
      rhu1=rhb(ityhb)/rda
      rhu2=rda/rhb(ityhb)
      teta0 = vhbtet0(ityhb)*dgrrdn
      coshu=cos(hhb(i1))-cos(teta0)
      cos2=coshu*coshu
      exphu1=exp(-vhb1(ityhb)*boa)
      exphu2=exp(-vhb2(ityhb)*(rhu1+rhu2-2.0))
      ehbh=(1.0-exphu1)*dehb(ityhb)*exphu2*cos2
      ehb=ehb+ehbh
      write(*,*)  " j3 j1 j2 j4 ",j(1),j(2),j(3),j(4)
      write (*,*) "     H----Z", rda
      write (*,*) "     BOXH  ", boa
      write (*,*) "     Teta, cosT  ", hhb(i1)/dgrrdn,coshu
      write (*,*) "     Teta0    ", vhbtet0(ityhb)
      write (*,*) "     phb1 phb2 phb3
     $",dehb(ityhb),vhb1(ityhb),vhb2(ityhb)  
      write (*,*) "   1-exp1    ", 1-exphu1
      write (*,*) "     exp2    ", exphu2
      write (*,*) "     ehbh    ", ehbh
...
*     dcell(k1,j(3),kcell)=dcell(k1,j(3),kcell)+dehbdrda*drda(k1)       !Hasan Metin Aktulga suggestion, 10/3/2008
      end do
...
*      h2=exp(-p3*rr+p5)
*      ewh=p2*h1*h2
*      dewdr=p2*h2*(2*p4*rr-h1*p3)


*1. X6S potential function 
      p1=p1co(ity1,ity2)
      p2=p2co(ity1,ity2)
      p3=p3co(ity1,ity2)
      p4=p4co(ity1,ity2)
      ...
      p7=p7co(ity1,ity2)
      if (rr.gt.0.80) then
      exp1 = p2*exp(-p1*rr+p3)
      exp2 = exp(-p4*rr**p5+p6*rr+p7)
      ewh = exp1*exp2
      dewdr = -p1*exp1*exp2 - (p4*p5*rr**(p5-1.0)-p6)*exp1*exp2
!      if (rr.lt.1.30) then
!      endif 
      else
      ewh = 0.0
...
*      h2=exp(-p3*rr+p5)
*      ewh=0.5*p2*h1*h2
*      dewdr=0.5*p2*h2*( 2*p4*rr-h1*p3)

*2. X6S potential function
      p1=p1co(ity1,ity2)
      p2=p2co(ity1,ity2)
      p3=p3co(ity1,ity2)
      p4=p4co(ity1,ity2)
...
*      if (rr.gt.0.80) then
      exp1 = p2*exp(-p1*rr+p3)
      exp2 = 0.5*exp(-p4*rr**p5+p6*rr+p7)
      ewh = exp1*exp2
      dewdr = -p1*exp1*exp2 - (p4*p5*rr**(p5-1.0)-p6)*exp1*exp2
*      else
*      ewh = 0.0
...
* derivatives are not included yet!
*      if (rr.lt.0.80) then
*      edsip = 0.0
*      exp1 = -109.4853*exp(-4.0*(rr-0.74085));
*      exp2 = 1.0+4.0*(rr-0.74085)*(1.0+(rr-0.74085)*(1.0+(rr-0.74085)))
*      ewh = 0.5 * exp1 * exp2
*      endif
...
*      call dista2(ja1,ja2,rla,dx1,dy1,dz1)
* SN ReaxPQ
      call dista2(ja1,ja3,rla,dx1,dy1,dz1)
* End SN
